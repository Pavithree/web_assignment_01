...
\begin{enumerate}
    \item Based on the schematic representation from Figure \ref{fig:schematic_routing} create a routing table.
    \item Given the routing table from Table \ref{tab:routing_table} show the steps in path traversal if a packet is generated from the 67.0.0.0 network and heading for 26.0.0.0 network. 
\end{enumerate}
\begin{table}[h]
\centering
